...
class App {
    ...
    _sliderListener() {
        countryInfo.addEventListener("click", function(e) {
            if (e.target.closest("#slider")) section1.scrollIntoView({
                behavior: "smooth"
            });
        });
    }
    ...
    _getDataByCode = async function(code) {
        try {
            const countryData = await this._getJSON(`https://countries-api-836d.onrender.com/countries/alpha/${code}`, "Country code is invalid. Try again!");
    ...
    _getDataByName = async function(country) {
        try {
            const [countryData] = await this._getJSON(`https://countries-api-836d.onrender.com/countries/name/${country}`, "Country name is invalid. Try again!");
            this.#countryData = countryData;
            this.#coords = countryData.latlng;
            this._renderInfos();
        } catch (err) {
    ...
    _countryQuery(e) {
        ...
        // country code query
        try {
            if (data.length === 2) this._getDataByCode.call(this, data);
            // country name query
            ...
            if (Number.isFinite(+data[0])) {
                ...
                if (!validInputs(...coords)) throw new Error("Check your input. It can't be validate correctly ....");
    ...
    _clearCountryInfoField() {
        if (document.querySelectorAll(".country-info")) document.querySelectorAll(".country-info").forEach((e)=>e.remove());
    }
    _clearCountryField() {
    ...
    _loadInfo() {
        ...
        const html = `
    <h3 class="country-info">You are in ${data.name} <img class="country__info__img" src="${this.#countryData.flag}" />, ${data.region}.</h3>
    <h3 class="country-info margin-sm">${(+data.population / 1000000).toFixed(1)} million people live in there.</h3>
    <h3 class="country-info margin-md";>Currency is ${data.currencies[0].name}.</h3>
    <h3 class="country-info margin-lg";>Citizens speak ${data.languages[0].name}.</h3>
    <h3 class="country-info cursor-p" id="slider" >Check local time & weather below <span class="hand-click">\u{2935}\u{FE0F}</span></h3>
    `;
        countryInfo.insertAdjacentHTML("afterbegin", html);
    }
    _loadMap(position) {
    ...
    _getCountryData = async function(lat, lng) {
        ...
        https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lng}&format=json&apiKey=03622f340f754f8dabe96c41591b6149`);
            if (!geoData.results.length) throw new Error("Connection error (refresh the page)!!!!");
            const countryData = await this._getJSON(`https://countries-api-836d.onrender.com/countries/alpha/${geoData.results[0].country_code}`, "There is no country at this territory!");
            this.#countryData = countryData;
            this._renderCurrInfo();
            this._clearCountryInfoField();
            if (this.#countryData) delete this.#countryData[0];
    ...
    _getWeatherData = async function(lat, lng) {
        const weatherData = await this._getJSON(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&appid=2dfa6f845beab2346b8c0ccea8e5d024`);
    ...
    _renderCurrPopup(coords) {
        const marker = L.marker(coords).addTo(this.#map).bindPopup(L.popup({
            maxWidth: 100,
            minWidth: 100,
            // autoClose: false,
            closeOnClick: false
        })).setPopupContent(`<img class="country__img" style="width:10rem; height: 4.5rem" src="${this.#countryData.flag}" />`).openPopup();
        this.#markers.push(marker);
    }
    _getLocalTime(sec) {
        const date = new Date();
        date.setTime(date.getTime() + (sec * 1000 + date.getTimezoneOffset() * 60000));
        return [
    ...
    _renderWeatherData(weatherData) {
        ...
        const description = weatherData.weather[0].description[0].toUpperCase() + weatherData.weather[0].description.slice(1);
        const localTime = this._getLocalTime(weatherData.timezone);
        const html = `
    <h4 class="weather">Weather info:</h4>
    <p class="country__row"><span>\u{1F321}\u{FE0F}</span>${(+weatherData.main.temp - 273.15).toFixed()}\u{2103}</p>
    <p class="country__row"><span>\u{1F304}</span>${description}</p>
    <p class="country__row"><span>\u{1F343}</span>${+weatherData.wind.speed.toFixed()} m/sec</p>
    <h4 class="weather">Local time:</h4>
    <p class="country__row"><span>\u{231A}</span>${localTime[0]}</p>
    <p class="country__row"><span>\u{23F3}</span>${localTime[1]}</p>
    </div>`;
        document.querySelector(".country__data").insertAdjacentHTML("beforeend", html);
    }
    _renderCurrInfo() {
        this._clearMarkers();
        this._clearCountryField();
    ...
    <article class="country">
      ...
      <p class="country__row"><span>\u{1F307}</span>${data.capital}</p>
      <p class="country__row"><span>\u{1F46B}</span>${(+data.population / 1000000).toFixed(1)} people</p>
